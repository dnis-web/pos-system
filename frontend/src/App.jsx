...
function App() {
  ...
  const renderPage = () => {
    ...
    if (page === "facturacion") return <PaginaFacturacion token={session?.access_token} />
    if (page === "reportes") return <section><h2>Reportes</h2><p>Solo disponible para Admin</p></section>
  }
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        {menuItems.filter(m => m.roles.includes(rol)).map(m => (
          <button key={m.id} className={page === m.id ? "active" : ""} onClick={() => setPage(m.id)}>{m.label}</button>
        ))}
...
function PaginaProductos({ token }) {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/api/productos`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json()).then(data => { setProductos(Array.isArray(data) ? data : []); setCargando(false) })
  ...
  return (
    <section>
      <h2>Productos</h2>
      <table>
        <thead><tr><th>SKU</th><th>Nombre</th><th>Stock</th><th>Precio</th><th>Estado</th></tr></thead>
        <tbody>
          {productos.map(p => (
            <tr key={p.id}>
              <td>{p.sku || `PROD-${p.id?.slice(0,6)}`}</td>
              <td>{p.nombre}</td><td>{p.stock}</td><td>Q {p.precio_venta}</td>
...
function PaginaInventario({ token }) {
  ...
  return (
    <section>
      ...
      {alertas.length > 0 && <p style={{color: "red"}}>⚠️ {alertas.length} productos con stock bajo</p>}
      <table>
        <thead><tr><th>Producto</th><th>Stock actual</th><th>Stock mínimo</th><th>Estado</th></tr></thead>
        <tbody>
          {productos.map(p => (
            <tr key={p.id}>
              <td>{p.nombre}</td><td>{p.stock}</td><td>{p.stock_minimo}</td>
              <td><span className={idsAlerta.has(p.id) ? "badge warning" : "badge ok"}>{idsAlerta.has(p.id) ? "Stock bajo" : "Disponible"}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function PaginaVentas({ token }) {
  ...
  const productosFiltrados = productos.filter(p => p.nombre.toLowerCase().includes(busqueda.toLowerCase()) && p.activo)
  const agregarItem = (producto) => {
    const existe = items.find(i => i.producto_id === producto.id)
    if (existe) setItems(items.map(i => i.producto_id === producto.id ? {...i, cantidad: i.cantidad + 1} : i))
    else setItems([...items, { producto_id: producto.id, nombre: producto.nombre, precio_unitario: producto.precio_venta, cantidad: 1 }])
    setBusqueda("")
  }
  ...
  const finalizarVenta = async () => {
    ...
    if (data.venta) { setMensaje(`✅ Venta registrada #${data.venta.numero_venta}`); setItems([]) }
      ...
      {busqueda && (
        <ul style={{border: "1px solid #ccc", padding: "8px", listStyle: "none"}}>
          {productosFiltrados.slice(0, 5).map(p => (
            <li key={p.id} style={{cursor: "pointer", padding: "4px"}} onClick={() => agregarItem(p)}>{p.nombre} — Q{p.precio_venta}</li>
          ))}
        </ul>
      )}
      <table>
...
useEffect(() => {
  ...
  const emitirDTE = async (venta) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/mock-fel`, {
      ...
      body: JSON.stringify({ venta_id: venta.id, total: venta.total, secret: "pos-fel-secret-2026" })
    })
    const data = await res.json()
    setProcesando(false)
    ...
    <section>
      ...
      {dte && (
        <div style={{background: dte.error ? "#fee" : "#efe", padding: "12px", marginBottom: "16px", borderRadius: "8px"}}>
          {dte.error ? <p style={{color: "red"}}>❌ {dte.error}</p> : (
      ...
      <table>
        <thead><tr><th>Número</th><th>Fecha</th><th>Total</th><th>Método</th><th>DTE</th></tr></thead>
        <tbody>
          {ventas.map(v => (
            <tr key={v.id}>
              <td>{v.numero_venta}</td>
              <td>{new Date(v.creado_en).toLocaleDateString()}</td>
              <td>Q{v.total}</td><td>{v.metodo_pago}</td>
              <td><button onClick={() => emitirDTE(v)} disabled={procesando}>{procesando ? "Procesando..." : "Emitir DTE"}</button></td>
            </tr>
          ))}
        </tbody>
      </table>
